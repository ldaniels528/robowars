...
case class MachineGunShell(w: FxWorld, p: FxPoint3D, a: FxAngle3D, vel: FxPoint3D)
  extends AbstractShellCasing(w, p, a,
    computeVector(vel, a), FxAngle3D(2d, 2d, 2d), randomSpread = 1d, randomRotation = 2d, lifeTime = 1.5d) {

  // set the default polyhedron instance
  lazy val modelInstance = FxModelInstance("/models/weapons/shell.f3d", FxScale3D(0.05d, 0.05d, 0.05d))
